...
class Field {
	...
	public function callback_text( $args ) {
		...
		printf( '<input type="text" class="%1$s-text" id="%2$s" name="%2$s" value="%3$s"/>', esc_attr( $size ), esc_attr( $args['option_key'] ), esc_attr( $value ) );
		printf( '<span class="description"> %s </span>', esc_html( $this->get_desc() ) );
	...
	public function callback_checkbox( $args ) {
		...
		printf( '<input type="checkbox" class="checkbox" id="%1$s" name="%1$s" value="1" %3$s />', esc_attr( $args['option_key'] ), esc_attr( $value ), checked( $value, 1, false ) );
	...
	public function callback_multicheck( $args ) {
		...
		foreach ( $options as $key => $label ) {
			...
			printf( '<input type="checkbox" class="checkbox" id="%1$s[%2$s]" name="%1$s[%2$s]" value="%2$s" %3$s />', esc_attr( $args['option_key'] ), esc_attr( $key ), checked( $checked, $key, false ) );
			printf( '<label for="%1$s[%3$s]"> %2$s </label><br />', esc_attr( $args['option_key'] ), esc_html( $label ), esc_attr( $key ) );
		}

		printf( '<span class="description"> %s </span>', wp_kses_data( $this->get_desc() ) );
	...
	public function callback_radio( $args ) {
		...
		foreach ( $options as $key => $label ) {
			printf( '<input type="radio" class="radio" id="%1$s[%3$s]" name="%1$s" value="%3$s"%4$s />', esc_attr( $args['option_key'] ), esc_attr( $id ), esc_attr( $key ), checked( $value, $key, false ) );
			printf( '<label for="%1$s[%4$s]"> %3$s</label><br>', esc_attr( $args['option_key'] ), esc_attr( $id ), wp_kses_data( $label ), esc_attr( $key ) );
		}

		printf( '<span class="description"> %s</label>', wp_kses_data( $this->get_desc() ) );
	...
	public function callback_select( $args ) {
		...
		printf( '<select class="%1$s" name="%2$s" id="%2$s">', esc_attr( $size ), esc_attr( $args['option_key'] ), esc_attr( $id ) );
	...
	public function callback_textarea( $args ) {
		...
		printf( '<textarea rows="5" cols="55" class="%1$s-text" id="%2$s" name="%2$s">%3$s</textarea>', esc_attr( $size ), esc_attr( $args['option_key'] ), esc_attr( $value ) );
		printf( '<br /><span class="description"> %s </span>', wp_kses_data( $this->get_desc() ) );
	...
	public function callback_password( $args ) {
		...
		$value = esc_attr( $args['value'] );
		$size  = $this->get_size();
		printf( '<input type="password" class="%1$s-text" id="%2$s" name="%2$s" value="%3$s"/>', esc_attr( $size ), esc_attr( $args['option_key'] ), esc_attr( $value ) );
